import { Routes, User, APIMessage, DiscordAPIError, PermissionFlagsBits, Guild, GuildBasedChannel, Message, APIUser, Collection } from "discord.js";
...
export default class Giveaway {
  ...
  declare readonly Sponsor: string | undefined;
  ...
  async setSiteData(): Promise<void> {
  ...
  userIsParticipant(userId: string): boolean {
    return this.Participants.has(userId);
  }

  async setTimer(): Promise<boolean> {
    ...
    if (timeMs > 2147483647) // setTimeout limit
      return await this.watchOverTimeout();

    this.timeout = setTimeout(async () => await this.start(), timeMs);
    return true;
  }

  async watchOverTimeout() {
    if (((this.DateNow + this.TimeMs) - Date.now()) < 2147483647)
      await this.setTimer();
    else setTimeout(async () => await this.watchOverTimeout(), 1000 * 60 * 60);
    return true;
  }
  ...
  async refresh(): Promise<boolean> {
    if (this.timeout)
      clearTimeout(this.timeout);
    return await this.setTimer();
  }

  retry(): void {
    this.retryCooldown = (this.retryCooldown || 0) + 1000;
    setTimeout(async () => await this.start(), this.retryCooldown);
    return;
  }
  ...
  async fetchMessage(): Promise<DiscordAPIError | APIMessage> {
  ...
  async getChannel(): Promise<GuildBasedChannel | null | undefined | DiscordAPIError> {
  ...
  async start() {
    ...
    if (!clientMember) return this.delete();
    ...
    return await lauch(this as any);
  }

  async getMessage(): Promise<Message<true> | undefined> {
    if (this.message) return this.message;
    if (!this.channel || !("messages" in this.channel)) return this.message;
