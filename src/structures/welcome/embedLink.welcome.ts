...
export default async function embedWelcomeLink(
  ...
  if (interaction instanceof StringSelectMenuInteraction)
    return await interaction.showModal(modals.welcome.messageLink(locale));
  ...
  try {
    ...
    if (messageFetched.embeds.length > 1) {
      ...
      return await resetMessageAndReply(t("welcome.content.more_than_one_embed", { e, locale, embedCommand }));
    }
    ...
    return await interaction.editReply({
      ...
      .then(() => WelcomeCacheEmbed.set(member.id, embed));
