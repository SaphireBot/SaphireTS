...
export default async function staffData(socket: Socket) {
    ...
    const tagsAndIds = {
        ...
        staff: ["611006830411251714", "781137239194468403", "830226550116057149", "327496267007787008", "144943581965189120", "435444989695229952", "674295513742442573"], // Ally, Dspofu, Pepy, San, Serginho, Yafyr, Liege
    };
    ...
    const staffers = await Promise.all(
        ...
        .then(users => users
            .filter(Boolean)
            .filter(user => !user.username.includes("deleted_user")),
        );
