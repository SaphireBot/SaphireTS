...
export default class TwitchWebsocket extends EventEmitter {
  ...
  async checkExistingStreamers(streamers: string[]): Promise<UserData[] | null | { message: string }> {
    ...
    let response: UserData[] | null = null; // await socket.emitWithAck("twitch", 2000, "fetch", null, url);
  ...
  async disable(streamer: string, channelId: string): Promise<boolean | null | { message: string }> {
    ...
    let response: boolean | null | { message: string } = null; // await socket.emitWithAck("twitch", 1000, "disable", null, { streamer, channelId });
  ...
  async getClips(streamerId: string): Promise<Clip[] | null> {
    ...
    let response: Clip[] | null = null;  //await socket.emitWithAck("twitch", 2000, "fetch", null, `https://api.twitch.tv/helix/clips?broadcaster_id=${streamerId}&first=25`);
  ...
  async getClip(clipId: string): Promise<Clip[] | null> {
    ...
    let response: Clip[] | null = null; // await socket.emitWithAck("twitch", 2000, "fetch", null, `https://api.twitch.tv/helix/clips?id=${clipId}`);
  ...
  async getData(): Promise<TwitchClassData | null> {
    ...
    let response: TwitchClassData | null = null; // await socket.emitWithAck("twitch", 2000, "data", null, "get");
