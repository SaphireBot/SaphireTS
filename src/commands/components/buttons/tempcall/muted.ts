import { ButtonInteraction, ChannelType, PermissionFlagsBits, Collection, GuildMember } from "discord.js";
...
export default async function muted(interaction: ButtonInteraction<"cached">) {
    ...
    TempcallManager.guildsWithMuteCount.has(guildId)
        ? TempcallManager.guildsWithMuteCount.delete(guildId)
        : TempcallManager.guildsWithMuteCount.add(guildId);
