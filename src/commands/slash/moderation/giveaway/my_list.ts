import { APIEmbed, ButtonStyle, ChatInputCommandInteraction, Colors, Message } from "discord.js";
...
export default async function mylist(
  ...
  const giveaways = await GiveawayManager.fetchGiveawayFromAnUser(user.id);

  if (!giveaways?.length)
    return await interaction.reply({ content: t("giveaway.my_list_giveaway_found", { e, locale }) });
  ...
  function EmbedGenerator(array: string[]): APIEmbed[] {
    ...
    const footer_text = t("giveaway.my_list", { e, locale, giveaways: giveaways.length });

    for (let i = 0; i < array.length; i += 10) {

      embeds.push({
        color: Colors.Blue,
        title: t("giveaway.my_list_title", {
          e,
          locale,
