...
export default async function edit(
  ...
  const character = await Database.CharactersCache.findOne({ pathname: embed.footer.text });
  ...
  if (data.id === "langs") {
    ...
    return await interaction.showModal(
      modals.characterEditLanguage(data)
    );
  }

  const value = values[0] as "base_data" | "another_answers" | "language";
