import { APIApplicationCommand, APIEmbed, ButtonInteraction, Colors, StringSelectMenuInteraction } from "discord.js";
...
export default async function slash(interaction: StringSelectMenuInteraction) {
  ...
  function EmbedGenerator(array: APIApplicationCommand[]) {
    ...
    for (let i = 0; i < array.length; i += 10) {
      ...
      embeds.push({
        color: Colors.Blue,
        title: t("help.embeds.slash.title", { e, locale, pagination }),
        url: urls.saphireSiteUrl + "/commands",
        description: t("help.embeds.slash.description", locale),
        fields,
        footer: {
          text: t("help.embeds.slash.footer", { commands: commands.length, locale }),
        },
      });
