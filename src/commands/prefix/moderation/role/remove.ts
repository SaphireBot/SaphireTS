...
export default async function removeRole(message: Message<true>) {
  ...
  if (!roles.size)
    return await message.reply({ content: t("role.remove.no_roles_mentioned", { e, locale }) });

  if (!members.length)
    return await message.reply({ content: t("role.remove.no_members_mentioned", { e, locale }) });
  ...
  const msg = await message.reply({ content: t("role.remove.removing", { e, locale }) });
