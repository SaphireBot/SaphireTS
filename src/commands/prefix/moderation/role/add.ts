...
export default async function addRole(message: Message<true>, args: string[]) {
  ...
  if (!queries?.length)
    return await message.reply({ content: t("role.add.invalid_params", { e, locale }) });

  const msg = await message.reply({ content: t("role.add.loading", { e, locale }) });
  ...
  if (!roles.size)
    return await msg.edit({ content: t("role.add.no_roles_mentioned", { e, locale }) });

  if (!members.size)
    return await msg.edit({ content: t("role.add.no_members_mentioned", { e, locale }) });
  ...
  await msg.edit({ content: t("role.add.adding", { e, locale }) });
